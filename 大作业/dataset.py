...
class TestDataset(Dataset):
    def __init__(self, data_path, input1_name='input1', input2_name='input2'):
        # Support custom input folder names for flexible testing
        self.width = 512
        self.height = 512
        ...
        for data in sorted(datas):
            data_name = data.split('/')[-1]
            # Check if folder matches specified input names
            if data_name == self.input1_name or data_name == self.input2_name:
                # Map custom names to input1/input2 for internal consistency
                if data_name == self.input1_name:
                    key = 'input1'
                else:
                    key = 'input2'
                self.datas[key] = {}
                self.datas[key]['path'] = data
                self.datas[key]['image'] = glob.glob(os.path.join(data, '*.jpg'))
                self.datas[key]['image'].sort()
        print(f'Loaded test data: {self.datas.keys()} from folders {self.input1_name} and {self.input2_name}')
