...
sys.path.insert(0, os.path.join(current_dir, 'UDIS2/Warp/Codes'))
...
MODEL_DIR = os.path.join(os.path.dirname(__file__), 'UDIS2/Warp/model')
...
def test(args):
    ...
    # Use output_path if provided, otherwise use test_path
    output_base = args.output_path if args.output_path else args.test_path
    if not os.path.exists(output_base):
        os.makedirs(output_base)
        print(f'Created output directory: {output_base}')

    path_ave_fusion = os.path.join(output_base, 'ave_fusion/')
    if not os.path.exists(path_ave_fusion):
        os.makedirs(path_ave_fusion)
    path_warp1 = os.path.join(output_base, 'warp1/')
    if not os.path.exists(path_warp1):
        os.makedirs(path_warp1)
    path_warp2 = os.path.join(output_base, 'warp2/')
    if not os.path.exists(path_warp2):
        os.makedirs(path_warp2)
    path_mask1 = os.path.join(output_base, 'mask1/')
    if not os.path.exists(path_mask1):
        os.makedirs(path_mask1)
    path_mask2 = os.path.join(output_base, 'mask2/')
...
if __name__=="__main__":
    ...
    parser.add_argument('--test_path', type=str, default='/opt/data/private/nl/Data/UDIS-D/testing/')    # Add arguments for custom input folder names
    ...
    parser.add_argument('--output_path', type=str, default='', help='Path to save output results (default: same as test_path)')
